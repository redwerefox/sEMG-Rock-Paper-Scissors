...
###                        Input EMG data at time t as matrix (16,8)                                        ###
...
class neuralNetwork :
	...
	def __init__ (self, modelpath,userpath):
		...
		self.classifier = tf.estimator.Estimator(model_fn=sEMG_CNN_model_fn,params={"feature_columns":self.COLUMNS,
															"hidden_units":[64], "n_classes" : 3, "nn":self}
															,model_dir = modelpath)
	...
	def train (self, batch_size, num_epochs):
		for k in range(num_epochs):
			for i in range(1):
				...
				# Train the Model.
				train = self.classifier.train(
				input_fn=input_fn,
				steps=trainsteps)
				
				print("Accuarcy evaluation & loss {}".format(train))
			
			
		return self.evaluate(batch_size), train
...
def sEMG_CNN_model_fn(features,labels,mode,params):
	...
	dense = tf.layers.dense(inputs=pool_flat, units=512, activation=tf.sigmoid)
	dp1 = tf.layers.dropout(inputs=dense,rate=0.5, training=mode == tf.estimator.ModeKeys.TRAIN)
	...
	dp2 = tf.layers.dropout(inputs=dense2,rate=0.5, training=mode == tf.estimator.ModeKeys.TRAIN)
	...
	dp3 = tf.layers.dropout(inputs=dense3,rate=0.5, training=mode == tf.estimator.ModeKeys.TRAIN)
	"""
	...
	lossL2 = tf.add_n([tf.nn.l2_loss(v) for v in vars if 'bias' not in v.name]) * 0.0065
	loss += lossL2
	"""
...
def main():			
	...
	network = neuralNetwork ("./workspace/model_cx","./models/Vulpes/datasets")
	#steps caluclated for train anyway
	network.train(batch_size=100,num_epochs=1)	
